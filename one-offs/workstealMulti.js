...
wsim.hist = [ ]; // sequence of thread -> {node, ...}  partitionings
wsim.colors =
		(function () {
...
////////////////////////////////////////
	...
	function depthToOpacity(depth) {
		return Math.min((1.0) / (2 * maxDepth), 1);
	}
	...
	function makeThread (i) {
		...
		var took, gave, steps, stack, current;
	...
	//				current.style.padding= wsim.paddingSize;
		...
		var res = {
			...
			stepper: function (take, isDfs) {
				...
				wsim.hist[wsim.hist.length - 1][i].push(current)
	...
	function paintConflicts () {
		...
		for (var i = 0; i < wsim.nThreads; i++) {
			for (var n = 0; n < now[i].length; n++) {
				var o = now[i][n];
				var hit = false;
				for (var pn = 0; pn < prv[i].length; pn++) {
					if (o == prv[i][pn]) {
						hit = true;
						break;
					}
	...
	function reset () {
		step = 0;
		threads.forEach(function (t) { t.reset(); });
		traversed = 0;
	}
