  ...
  actions {
    loop childs {
      ...
      childs.i := 
        ...
        fixeda 
        ..         
        childs$-.i  + childs$i.fixedb + childs$$.count;
    }
  }
}
\end{lstlisting}

Our reduction unrolls the loop into the following:
\begin{lstlisting}
interface TopI {
  var i : int;
  input fixeda : int;
...
class TopC : TopI {
  attributes {
    var childs0_count, childs1_count, childs2_count, childsn_count : int;
    var childs0_i, childs1_i, childs2_i, childsn_i : int;
  }
  children {  
    childs0, childs1, childs2, childsn : TopI;
  }
  actions {   
    childs0_count := f();
    childs0.count := f(childs0_count);
    childs1_count := f(childs0_count);
    childs1.count := f(childs1_count);
    childs2_count := f(childs1_count);
    childs2.count := f(childs2_count);
    childsn_count := f(childs2_count);
    childsn.count := f(childsn_count);

    childs0_i := f(fixeda, childs0_fixedb, childsn_count);
    childs0.i := f(childs0_i);
    childs1_i := f(childs0_i, childs1.fixedb);
    childs1.i := f(childs1_i);
    childs2_i := f(childs1_i, childs2.fixedb);
    childs2.i := f(childs2_i);
    childsn_i := f(childs2_i, childsn.fixedb);
    childsn.i := f(childs2_n);
  }
}        
\end{lstlisting}
