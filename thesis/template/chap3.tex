...
\item \textbf{Separate Loops over the Same Collection} Loops over the same collection may still need to be separated. Consider the following loop:
\begin{lstlisting}[mathescape]
loop childs {
  childs.a := fold 0 .. childs$\$-$.b + 1;
...
\end{lstlisting}
Note that translation of references of the form ``\code{childs$-.fld}'' require tracking the loop step in order to pick whether to use the initial value or the previous node's value.
