...
The result of the rewriting is a canonical attribute grammar without loops. Our full implementation differs in two significant ways. First, it performs static checks such as that step variable ``\code{child$i.fld}'' is not referred to by the \code{fold} initialization expression. Second, it supports loops that may temporarily escape as part of a \code{recursive} traversal. Each loop step over an element may require traversal into the element's subtree, so we expand child attributes with a local and transfer version in order to reason about safe placement of the recursive call.
...
The process is guaranteed to terminate. First, moving statements out of one loop completes in time linear in the size of the range of the loop. Second, the finite number of loops means that the process must only repeat a finite number of times.
\item \textbf{Separate Loops over Different Collections} Loops over different sets 
