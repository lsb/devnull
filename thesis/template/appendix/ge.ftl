...
schedule { 
   "P = [(_,td,_,_,_),(_,bu,_,_,_),(_,td,_,_,_)]"
}
interface IRoot { 
...
class Second (propagateIntermediate,propagateChilds) : SecondI {
  ...
  //background
  @render @RectangleZ(xOffset $-$ radius, yOffset $-$ radius, 
    4 * radius, 4 * radius, $-$0.1f, rgb(0,0,0));
  w := 10;
    loop childs {
  ...
  actions {
    ...
    loop childs {
      ...
      childs.tween := 
        ...
        ? $-$1.0f 
        : ((childs.idx $-$ tweenMin * childs$\$$$\$$.idx) 
          / (tweenMax * numSpikes $-$ tweenMin * numSpikes));
        
      childs.isFirst := 
        ((childs$\$$i.idx >= tweenMin * numSpikes) 
          && ((childs$\$$i.idx $-$ 1) < tweenMin * numSpikes)) ? 1 : 0;
      childs.isLast := 
        ((childs$\$$i.idx <= tweenMax * numSpikes) 
          && ((childs$\$$i.idx + 1) > tweenMax * numSpikes)) ? 1 : 0;
